...
export default function ManagerDashboard() {
    ...
    const handleSubmit = async () => {
        ...
        for (let i = 0; i < formProduct.variants.length; i++) {
            ...
            if (!v.size || !v.color || !v.originalPrice || !v.currentPrice || !v.stock || !v.imageFile) {
                alert(`Vui lòng điền đầy đủ thông tin biến thể ${i + 1} và chọn ảnh!`);
                return;
            }
        }
        ...
        const variantsPayload = formProduct.variants.map(v => ({
            size: v.size,
            color: v.color,
        ...
        try {
            console.log("🚀 Sending product data:", {
                ...
                files: formProduct.variants.map(v => v.imageFile ? v.imageFile.name : 'No file')
            });
    ...
    const handleUpdate = async () => {
        ...
        // Gửi biến thể dưới dạng array JSON
        const variantsPayload = formProduct.variants.map(v => ({
            _id: v._id,
            size: v.size,
            color: v.color,
